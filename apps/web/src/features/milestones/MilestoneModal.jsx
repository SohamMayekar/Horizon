    ...
    <AnimatePresence>
      {isOpen ? (
        <div style={{ position: "fixed", inset: 0, zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          ...
          <motion.div
            style={{ position: "relative", zIndex: 1, width: "100%", maxWidth: 460, background: "#0B0D18", border: `1px solid ${cat.color}35`, borderRadius: 22, boxShadow: `0 0 0 1px #1C2030, 0 40px 100px rgba(0,0,0,0.8), 0 0 80px ${cat.color}12`, overflow: "hidden" }}
          ...
          >
            ...
            <div className="px-5 py-5 flex flex-col gap-5">
              <Field label="Title">
                <input autoFocus type="text" placeholder="e.g. Buy a flat, Wedding…" value={form.title} onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))} style={{ outline: "none" }} className="w-full bg-[#080A13] border border-[#1C2030] rounded-xl px-4 py-2.5 text-sm text-slate-100 placeholder:text-slate-600 focus:border-blue-500/60 transition-all" />
              </Field>
              <div className="grid grid-cols-2 gap-4">
                <Field label={`Age — ${form.age}`}>
                  <div className="relative h-10 flex items-center">
                    <div className="absolute inset-x-0 h-[3px] rounded-full" style={{ background: "#1C2030" }} />
                ...
                <Field label="Cost (₹)">
                  <div className="relative">
                    ...
                    <input type="number" placeholder="0" value={form.cost} onChange={(e) => setForm((f) => ({ ...f, cost: e.target.value }))} style={{ outline: "none" }} className="w-full bg-[#080A13] border border-[#1C2030] rounded-xl pl-7 pr-3 py-2.5 text-sm text-slate-100 placeholder:text-slate-600 focus:border-blue-500/60 transition-all tabular-nums" />
                  </div>
                </Field>
              </div>
              <Field label="Category">
                <div className="grid grid-cols-4 gap-1.5">
                  {CATEGORY_KEYS.map((key) => {
                    ...
                    return (
                      <button key={key} onClick={() => setForm((f) => ({ ...f, category: key }))} className="flex flex-col items-center gap-1.5 py-2.5 px-1 rounded-xl transition-all" style={{ background: active ? c.dim : "#0E1118", border: `1px solid ${active ? c.color + "55" : "#1C2030"}`, boxShadow: active ? `0 0 12px ${c.color}20` : "none" }}>
                        <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: active ? `${c.color}28` : "#1A1F2E" }}>
            ...
            <div className="flex items-center justify-between gap-3 px-5 py-4 border-t border-[#1A1F2E]">
              {!isNew ? (
              ...
              <div className="flex items-center gap-2">
                <button onClick={closeModal} className="px-4 py-2 rounded-xl text-xs font-bold text-slate-400 hover:text-slate-200 bg-[#1A1F2E] hover:bg-[#222840] transition-all">Cancel</button>
